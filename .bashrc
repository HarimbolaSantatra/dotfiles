...
# Alias pour le python par defaut
# alias pydef='/usr/bin/python3'
...
#Spring CLI
export PATH=/opt/spring-3.0.1/bin:$PATH
# AVRA
export PATH=/opt/avra-1.3.0/src:$PATH
# AVRGCC & AVRDUDE
...
# Set terminal tab name: method 1
function titre(){
	if [ -z "$ORIG" ]; then ORIG=$PS1
	fi
	TITLE="\[\e]2;$@\a\]"
	PS1=${ORIG}${TITLE}
};
...
# Prevent text from truncating in terminal
...
trash()
{
        mv "$1" /home/santatra/.local/share/Trash/files/
}

# copy directory path to clipboard using xsel
ydxsel()
{
    dir=$(pwd);
    echo "$dir" | xsel -b;
